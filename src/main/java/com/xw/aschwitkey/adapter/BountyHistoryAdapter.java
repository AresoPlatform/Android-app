...
import com.xw.aschwitkey.entity.BountyHistoryBean;
...
public class BountyHistoryAdapter extends RecyclerView.Adapter<BountyHistoryAdapter.ViewHolder> {
    private Context context;
    private List<BountyHistoryBean.Result> list;
    private LayoutInflater inflater;

    public BountyHistoryAdapter(Context context, List<BountyHistoryBean.Result> list) {
    ...
    @NonNull
    @Override
    public BountyHistoryAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ViewHolder(inflater.inflate(R.layout.item_bbs_reward_history_layout, parent, false));
    }

    @Override
    public void onBindViewHolder(@NonNull BountyHistoryAdapter.ViewHolder holder, int position) {
        holder.mTextView_rewardName.setText(list.get(position).getFromName());
        holder.mTextView_title.setText("打赏了 " + list.get(position).getTitle());
        holder.mTextView_reward.setText("+ " + list.get(position).getRewardAmount().setScale(3, BigDecimal.ROUND_DOWN).toString()+" xas");
