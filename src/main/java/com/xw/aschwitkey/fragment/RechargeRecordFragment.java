...
import com.xw.aschwitkey.adapter.DdzRechargeHistoryAdapter;
...
public class RechargeRecordFragment extends NewLazyFragment implements OnRefreshListener, OnLoadMoreListener {
    ...
    private Gson gson;
    private DdzRechargeHistoryAdapter adapter;
    ...
    public void initData() {
        list = new ArrayList<>();
        gson = new Gson();
        adapter = new DdzRechargeHistoryAdapter(mContext, list);
            ...
            public void run() {
                try {
                    ...
                    mContext.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            myDialog = DialogUtils.createLoadingDialog(mContext, "获取充值记录...");
                            payLists();
                        }
                    });
    ...
    private void payLists() {
        ...
        OkHttpClient.initPost(Http.payLists, requestBodyPost).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                mContext.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        ToastUtil.showShort(mContext,"网络错误，获取充值记录失败");
                    }
                });
            }
                    ...
                    public void run() {
                        try {
                            ...
                            if (jsonObject.getInt("code")==1){
                                DdzRechargeBean bean = gson.fromJson(jsonObject.getString("data"),DdzRechargeBean.class);
                                list.clear();
                            ...
                            ToastUtil.showShort(mContext, "获取充值记录错误："+e.getMessage());
                        }
                    }
                });
            }
